...
kmeans_clusters = splines_kmeans(
    log(preData[1:500,]+1), moanin_model=moaninObject, n_clusters=3,
    random_seed=42,
    n_init=20)
```
...
Because there is variability in how well the genes fit a cluster, we would like to be able to score how well a gene fits a cluster. Furthermore, we often chose a subset of genes based on our filter, and we would
...
```{r, eval=FALSE}
scores_and_labels = splines_kmeans_score_and_label(data=preData, kmeans_clusters)
```
