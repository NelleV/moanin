---
title: "documentation of timecourse DE"
...
$$\beta_0=(X'X)^{-1}-(X'X)^{-1}C'\left( C(X'X)^{-1}C'\right)^{-1}C(X'X)^{-1}=\hat\beta - W\hat\beta$$
where
...
$$\lambda=2 \frac{\ell(\hat\beta,\hat\sigma)}{\ell(\hat\beta_0,\hat\sigma_0)}=n\log\frac{RSS_{null}}{RSS_{full}}\sim \chi^2_{k}$$
...
$$\hat\sigma=\frac{1}{n}||Y-X\hat\beta||^2$$

However, both of these definitions appear to be different from the current implementations.
